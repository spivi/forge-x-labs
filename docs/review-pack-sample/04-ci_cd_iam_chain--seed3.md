# Review Sheet 04 - `ci_cd_iam_chain--seed3`
...
- `vpc-staging` - **VPC** env-staging-vpc-alt (low)
- `subnet-public-a` - **Subnet** app-public-a-b (medium)
- `sg-web` - **SecurityGroup** app-web-sg-r1 (high)
- `cicd-github` - **CICDIdentity** svc-github-actions-oidc-v2 (high)
- `role-deploy` - **IAMRole** DeployRole-x (high)
- `role-runtime` - **IAMRole** res-RuntimeRole (critical)
- `pol-deploy-passrole` - **IAMPolicy** app-DeployPassRolePolicy-alt (high)
- `pol-runtime-s3read` - **IAMPolicy** res-RuntimeS3ReadPolicy (high)
- `s3-customer-exports` - **S3Bucket** svc-customer-exports-b (critical)
- `s3-public-assets` - **S3Bucket** env-public-assets-alt (medium)
- `app-analytics-exporter` - **Application** env-analytics-exporter-v2 (medium)
- `data-customer-exports` - **DataSet** env-customer-export-data-x (critical)
- `trail-main` - **LogTrail** svc-main-trail-x (medium)
- `subnet-benign-extra` - **Subnet** spare-subnet (low)
