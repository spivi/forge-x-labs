# Review Sheet 05 — `ci_cd_iam_chain--seed4`
...
- `acct-main` — **Account** svc-staging-account-b (medium)
- `vpc-staging` — **VPC** svc-staging-vpc (low)
- `subnet-public-a` — **Subnet** env-public-a-b (medium)
- `sg-web` — **SecurityGroup** res-web-sg-v2 (high)
- `cicd-github` — **CICDIdentity** github-actions-oidc-x (high)
- `role-deploy` — **IAMRole** svc-DeployRole-b (high)
- `role-runtime` — **IAMRole** env-RuntimeRole-b (critical)
- `pol-deploy-passrole` — **IAMPolicy** svc-DeployPassRolePolicy-v2 (high)
- `pol-runtime-s3read` — **IAMPolicy** env-RuntimeS3ReadPolicy-b (high)
- `s3-customer-exports` — **S3Bucket** env-customer-exports-v2 (critical)
- `s3-public-assets` — **S3Bucket** app-public-assets-alt (medium)
- `app-analytics-exporter` — **Application** res-analytics-exporter (medium)
- `data-customer-exports` — **DataSet** res-customer-export-data-r1 (critical)
- `trail-main` — **LogTrail** svc-main-trail-x (medium)
- `subnet-benign-extra` — **Subnet** svc-spare-subnet-x (low)
