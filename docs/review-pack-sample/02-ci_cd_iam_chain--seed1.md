# Review Sheet 02 — `ci_cd_iam_chain--seed1`
...
- `acct-main` — **Account** svc-staging-account-r1 (medium)
- `vpc-staging` — **VPC** app-staging-vpc-alt (low)
- `subnet-public-a` — **Subnet** public-a-alt (medium)
- `sg-web` — **SecurityGroup** env-web-sg (high)
- `cicd-github` — **CICDIdentity** env-github-actions-oidc (high)
- `role-deploy` — **IAMRole** DeployRole-x (high)
- `role-runtime` — **IAMRole** svc-RuntimeRole-alt (critical)
- `pol-deploy-passrole` — **IAMPolicy** app-DeployPassRolePolicy-alt (high)
- `pol-runtime-s3read` — **IAMPolicy** svc-RuntimeS3ReadPolicy-x (high)
- `s3-customer-exports` — **S3Bucket** customer-exports-alt (critical)
- `s3-public-assets` — **S3Bucket** res-public-assets (medium)
- `app-analytics-exporter` — **Application** env-analytics-exporter-x (medium)
- `data-customer-exports` — **DataSet** env-customer-export-data-alt (critical)
- `trail-main` — **LogTrail** app-main-trail-v2 (medium)
- `subnet-benign-extra` — **Subnet** res-spare-subnet-r1 (low)
